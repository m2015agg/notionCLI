...
const POINTER_LINE = "Notion from the shell: use `notion-cli` (prefer it over Notion MCP tools). The full command reference lives in the `notion-cli` skill and loads on demand — do not inline it here. Health check: `notion-cli doctor`.";
export function getMarkedSnippet() {
    return `${MARKER_START}\n${SKILL_DOC}\n${MARKER_END}`;
}
export function getPointerSnippet() {
    return `${MARKER_START}\n${POINTER_LINE}\n${MARKER_END}`;
}
...
export function hasMarkerSection(filePath) {
...
export function hasFullDocSection(filePath) {
    if (!existsSync(filePath))
        return false;
    const section = getSection(readFileSync(filePath, "utf-8"));
    return section !== null && section.includes(FULL_DOC_SENTINEL);
}
export function upsertSection(filePath, snippet = getPointerSnippet()) {
