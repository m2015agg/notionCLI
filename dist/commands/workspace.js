...
import { openDb, hasDb, searchWorkspace, getAllPages, getAllDatabases, getDbProperties, findDatabaseByQuery, buildTree, getBreadcrumb, } from "../util/workspace-db.js";
...
export function workspaceCommand() {
    ...
    cmd
        ...
        try {
            ...
            const filterDbOnly = (nodes) => {
                if (!opts.databasesOnly)
                    return nodes;
                const filtered = [];
            ...
            console.log("\n  \uD83D\uDCC1 Workspace");
            const printNode = (node, prefix, isLast, depth) => {
                if (depth > maxDepth)
                    return;
    ...
    cmd
        ...
        try {
            ...
            const allPages = getAllPages(db);
            const matchedPage = allPages.find((p) => p.title === topResult.name || p.title.toLowerCase().includes(query.toLowerCase()));
            if (!matchedPage) {
