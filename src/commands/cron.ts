...
function isSnapshotHook(hook: unknown): boolean {
  return (
    isPlainObject(hook) &&
...
function entryHasSnapshotHook(entry: unknown): boolean {
  return (
    isPlainObject(entry) &&
...
function loadSettings(settingsPath: string): Record<string, unknown> | null {
  ...
  try {
    ...
    write(`\n  Warning: could not parse ${settingsPath} (${(e as Error).message}) — aborting without changes.\n\n`);
    return null;
  }
}
 ...
 */
export function removeSnapshotHook(settingsPath: string): "removed" | "not_found" {
  if (!existsSync(settingsPath)) return "not_found";
  let settings: Record<string, unknown>;
  try {
    const parsed: unknown = JSON.parse(readFileSync(settingsPath, "utf-8"));
    if (!isPlainObject(parsed)) return "not_found";
    settings = parsed;
  } catch {
    return "not_found";
  }

  const hooksVal: unknown = settings.hooks;
  if (!isPlainObject(hooksVal)) return "not_found";
  const sessionStartVal: unknown = hooksVal.SessionStart;
  if (!Array.isArray(sessionStartVal)) return "not_found";
  if (!sessionStartVal.some(entryHasSnapshotHook)) return "not_found";

  const kept = sessionStartVal
    .map((entry) => {
      if (!entryHasSnapshotHook(entry)) return entry;
      const e = entry as Record<string, unknown>;
      return { ...e, hooks: (e.hooks as unknown[]).filter((h) => !isSnapshotHook(h)) };
    })
...
function handleHook(opts: { status?: boolean; remove?: boolean }): void {
...
export function cronCommand(): Command {
  return new Command("cron")
    ...
    .action((opts: { time: string; status?: boolean; remove?: boolean; hook?: boolean }) => {
      if (opts.hook) {
        handleHook(opts);
        return;
      }
      ...
      try {
        ...
        write(`\n  Failed to set cron: ${(e as Error).message}\n\n`);
      }
    });
}
